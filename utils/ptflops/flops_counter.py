...
def get_model_complexity_info(model, input_res,
    ...
    if input_constructor:
        ...
        _ = flops_model(batch)
    flops_count, params_count, cycles_count = flops_model.compute_average_flops_cost()
    if cycles_count != 0:
        print('cycle count only for conv: {} cycles'.format(params_to_string(cycles_count, precision=3)))
...
def compute_average_flops_cost(self):
    ...
    for module in self.modules():
        if is_supported_instance(module):
            ...
            cycles_sum += module.__cycles__
    params_sum = get_model_parameters_number(self)
    return flops_sum / batches_count, params_sum, cycles_sum / batches_count
...
def conv_bp_cycle_counter_hook(conv_module, input, output):
    # Can have multiple inputs, getting the first one
    # get cycle
    input = input[0]
    ...
    overall_conv_flops = conv_per_position_flops * active_elements_count
    # group size g, flops y, average cycle x,   cycle_total = y/g * x
    # TODO: skip zero weight by sparse encode and decode
    overall_cycles = 0
    if conv_module.weight_int is not None:
        from models._modules.bit_pruner import count_bit
        bit_cnt = count_bit(conv_module.weight_int)
        if groups == 1:
            pass
        else:
            assert filters_per_channel == 1, 'only support depth wise convolution group==out_channel==in_channel'
            bit_cnt.transpose_(0, 1)
        pe_size = 32
        total_cycles = 0
        (out_channel, in_channel, k, k) = bit_cnt.shape
        part_in_channel = math.ceil(in_channel / pe_size)
        for j in range(part_in_channel):
            bit_cnt_part = bit_cnt[:, j * pe_size: (j + 1) * pe_size, :, :]
            # out, in, k, k
            cycles = bit_cnt_part.max(axis=1)[0].sum()
            total_cycles += cycles.item()
        ave_cycles = total_cycles / bit_cnt.numel()
        overall_cycles = ave_cycles * overall_conv_flops
    ...
    conv_module.__flops__ += int(overall_flops)
    conv_module.__cycles__ += int(overall_cycles)
...
def add_flops_counter_variable_or_reset(module):
    if is_supported_instance(module):
        ...
        module.__flops__ = 0
        module.__cycles__ = 0  # for bit pruner
...
MODULES_MAPPING = {
    ...
    my_nn.Conv2dTruncation: conv_bp_cycle_counter_hook,
